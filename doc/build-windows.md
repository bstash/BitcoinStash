...
Below are some notes on how to build Bitcoin Stash for Windows.

The options known to work for building Bitcoin Stash on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Bitcoin Stash Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Bitcoin Stash applications.
...
Note that for WSL the Bitcoin Stash source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoinstash, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

Acquire the source in the usual way:

    git clone https://github.com/bstash/bitcoinstash.git
...
way. This will install to `c:\workspace\bitcoinstash`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoinstash/
