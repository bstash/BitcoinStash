...
void AuxMiningCheck(const Config &config)
{
  ...
  if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0
    ...
    throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                       "Bitcoin Stash is not connected!");
...
bool AuxMiningSubmitBlock(const Config &config,
...
UniValue createauxblock(const Config &config, const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
    ...
    return AuxMiningCreateBlock(config, scriptPubKey);
}

UniValue submitauxblock(const Config &config, const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
    ...
    return AuxMiningSubmitBlock(config,
